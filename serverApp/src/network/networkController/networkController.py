...
no errors occurred. See Errors for more information.
    ...
    arguments:
        key - an admin’s unique access key
        dict - should contain values for the following keys: id (32 digit uak string, must be unique among all keys), level
(string, either KEY_ROOT or KEY_PROVIDER), name (string, name of key owner).
    ...
    arguments:
        key - an admin’s unique access key
        id - either an unhashed 32 digit uak or a hashed id of an entry in the table
    ...
    arguments:
        key - an admin’s unique access key
        dict - may contain values for the following keys: id (string, must be unique among all keys), level
(string, either KEY_ROOT or KEY_PROVIDER), name (string, key owner).
    ...
    arguments:
        key - an admin’s unique access key
    returns: A NetworkObject whose error field and payload fields are set. The payload field will contain a list of
dictionaries, where each dictionary has a single keys’s information (id, level, name) if no errors occurred.
See Errors for more information.
    ...
    arguments:
        key - an admin’s unique access key
        id - the unhashed 32 digit uak or the hashed id of an entry in the table
    returns: A NetworkObject whose error field is set. See Errors for more information.
    errors:
        NO_ERROR - operation was successful
        ERROR_UNAUTHORIZED_OPERATION - invalid key
